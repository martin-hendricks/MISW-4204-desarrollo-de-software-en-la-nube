    ...
    def process_video(
        ...
        try:
            ...
            if not os.path.exists(output_path):
                raise VideoProcessingError(f"No se generó el video procesado: {output_path}")
            ...
            raise VideoProcessingError(f"Error procesando video con FFmpeg: {error_msg}")
    ...
    def add_intro_outro(
        ...
        try:
            ...
            if len(videos_to_concat) > 1:
                ...
                ffmpeg.run(output, overwrite_output=True, capture_stdout=True, capture_stderr=True, quiet=True)
