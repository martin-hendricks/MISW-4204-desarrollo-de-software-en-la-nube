...
def process_video(self, video_id: int) -> Dict:
    ...
    try:
        ...
        video.status = VideoStatus.PROCESSED  # Usar el enum en lugar de string
        ...
        # logger.info(f"   Tiempo total: ~{(datetime.now() - video.uploaded_at).seconds}s")
        # logger.info("=" * 60)
        ...
        if self.request.retries >= self.max_retries:
            logger.warning(f"💀 Video {video_id} falló definitivamente (permanece en estado 'uploaded')")
            logger.warning("   Para marcar como 'failed', actualizar ENUM video_status en PostgreSQL")
        raise
